...
import { getServiceBySlug, listServiceSlugs } from "@/content/services";
...
export async function generateImageMetadata() {
  return listServiceSlugs().map((slug) => ({
    id: slug,
    contentType,
    size,
  }));
...
export default async function ServiceOG({
  ...
  const content = getServiceBySlug(params.slug);
  const headline = content?.hero.h1 ?? "Roofing services in St. Louis";
